...
def plot_time_series(ws_series: np.ndarray, time_vals):
    ...
    fig.update_layout(
        **PLOTLY_LAYOUT,
        title=dict(text="Wind Speed Time Series", font=dict(color="#e8f4fd", size=14)),
...
def plot_wind_rose(ws_series: np.ndarray, wd_series: np.ndarray):
    ...
    fig.update_layout(
        ...
        title=dict(text="Wind Rose (Frequency by Direction & Speed)", font=dict(color="#e8f4fd", size=14)),
        legend=dict(
...
def plot_histogram(ws_series: np.ndarray):
    ...
    fig.update_layout(
        **PLOTLY_LAYOUT,
        title=dict(text="Wind Speed Distribution (Weibull Assessment)", font=dict(color="#e8f4fd", size=14)),
...
def main():
    ...
    with st.sidebar:
        ...
        st.divider()
        st.markdown("### 🔧 Parameter Turbin")
        st.markdown(
            '<div class="info-box" style="font-size:0.8rem;">Digunakan untuk estimasi potensi daya '
            '(power curve) di titik POI. Nilai default mengikuti turbin angin lepas pantai skala '
            'menengah-besar — sesuaikan jika perlu.</div>',
            unsafe_allow_html=True,
        )
        rotor_diameter = st.number_input(
            "Diameter rotor (m)", min_value=1.0, max_value=250.0, value=100.0, step=5.0,
            key="rotor_diameter",
        )
        air_density = st.number_input(
            "Massa jenis udara (kg/m³)", min_value=0.8, max_value=1.5, value=1.225, step=0.005,
            format="%.3f", key="air_density",
        )
        cp_value = st.slider(
            "Koefisien daya Cp", min_value=0.10, max_value=0.50, value=0.40, step=0.01,
            help="Batas Betz teoritis adalah 0.593; turbin modern umumnya beroperasi di 0.35–0.45.",
            key="cp_value",
        )
        ...
        st.divider()
        st.caption("Wind Energy Assessment · v1.3 · Built with Streamlit + xarray + Folium + Plotly")
    ...
    st.markdown(
        ...
        'with higher potential than the currently selected POI.</div>',
        unsafe_allow_html=True,
    )
    ...
    if leaderboard:
        ...
        st.markdown(
            "| Rank | Latitude | Longitude | Mean WS (m/s) | Max WS (m/s) | P75 WS (m/s) | Tier |\n"
            "|---|---|---|---|---|---|---|\n" +
            "\n".join(
                f"| {rank_labels[i]} | {e['lat']:.4f}° | {e['lon']:.4f}° | "
                f"{e['mean_ws']:.2f} | {e['max_ws']:.2f} | {e['p75_ws']:.2f} | "
                f"{'Excellent' if e['mean_ws'] >= 7.0 else 'Moderate' if e['mean_ws'] >= 5.0 else 'Low'} |"
