...
def build_screening_map(df, lat_key: str, lon_key: str,
    ...
    METRIC_LABELS = {
        "mean_ws_hub":   f"Mean WS @ {hub_height:.0f} m (m/s)",
        "p75_ws_hub":    f"P75 WS @ {hub_height:.0f} m (m/s)",
        "cf_proxy_pct":  "Productive Hours (%)",
        "mean_ws_10m":   "Mean WS @ 10 m (m/s)",
    }
    ...
    for tier in ["Excellent", "Moderate", "Low"]:
        ...
        fig.add_trace(go.Scattermapbox(
            ...
            hovertemplate=(
                ...
                f"Mean WS @ 10 m: %{{customdata[0]}} m/s<br>"
                f"Mean WS @ {hub_height:.0f} m: %{{customdata[1]}} m/s<br>"
                f"P75 WS @ {hub_height:.0f} m: %{{customdata[2]}} m/s<br>"
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown(
            ...
            'browse to select them or drag-and-drop straight onto the box below. The dashboard '
            'reads each file\'s metadata automatically and figures out whether it contains the '
            'U (eastward) wind component, the V (northward) component, or both — so files can be '
            'split by time, by component, or combined, in any mix. Not limited to ERA5: CMEMS, '
            'WRF, NOAA/NCEP, and other CF-style NetCDF sources are also recognised, and every '
            '.nc format version (classic NETCDF3, 64-bit offset, NETCDF4/HDF5) is supported.</div>',
            unsafe_allow_html=True,
        )
        uploaded_files = st.file_uploader(
            ...
            help="Drag-and-drop or browse for one or many .nc files. Any combination of U-only, "
                 "V-only, or combined U+V files is accepted, from any NetCDF-producing source.",
    ...
    with st.spinner(f"Reading and classifying {len(uploaded_files)} NetCDF file(s)…"):
        for f in uploaded_files:
            try:
                # Streamlit's UploadedFile is an in-memory buffer; xarray needs
                # a real path on disk. Each file gets its own temp path so
                # large batches never overwrite one another.
                suffix = os.path.splitext(f.name)[1] or ".nc"
                ...
                if kind == "unknown":
                    load_issues.append(
                        f"**{f.name}** — no recognisable U or V wind variable found "
        ...
        if not file_records:
            st.error("None of the uploaded files contained a recognisable U or V wind component.")
    ...
    with tab_overview:
        ...
        with kpi1:
            st.markdown(
                ...
                f'<div class="kpi-label">Mean WS @ 10 m</div>'
        ...
        with kpi2:
            st.markdown(
                ...
                f'<div class="kpi-label">Mean WS @ {hub_height:.0f} m hub</div>'
        ...
        with kpi3:
            st.markdown(
                f'<div class="kpi-card">'
                f'<div class="kpi-label">Max WS @ Hub</div>'
    ...
    with tab_screening:
        ...
        if screening_ok:
            ...
            for col, label, val, unit in [
                ...
                (sc4, "Domain Mean WS @ Hub", f"{scr_stats['domain_mean_hub']:.2f}",   "m/s"),
                (sc5, "Domain Best WS @ Hub", f"{scr_stats['domain_max_hub']:.2f}",    "m/s"),
            ]:
                with col:
                    st.markdown(
                        f'<div class="kpi-card">' 
            ...
            st.markdown(
                ...
                f'— mean WS <b>{scr_stats["best_mean_ws_hub"]:.2f} m/s</b> ' 
            ...
            with ctrl2:
                ...
                _metric_labels = [
                    f"Mean WS @ {hub_height:.0f} m hub",
                    f"P75 WS @ {hub_height:.0f} m hub",
                    "Productive Hours (%)",
                    "Mean WS @ 10 m (raw)",
                ]
            ...
            with hist_col:
                ...
                for tier, color in [("Excellent","#00d4ff"),("Moderate","#f0a500"),("Low","#3a5a7a")]:
                    ...
                    hist_fig.add_trace(go.Histogram(
                        ...
                        nbinsx=40,
                        hovertemplate=f"{tier}<br>WS: %{{x:.1f}} m/s<br>Count: %{{y}}<extra></extra>",
                    ))
                ...
                hist_fig.update_layout(
                    ...
                    xaxis=dict(gridcolor="#1a2d45", linecolor="#1e3a5f",
                               title=f"Mean WS @ {hub_height:.0f} m (m/s)"),
            ...
            # ── Top-10 sites table ─────────────────────
            with st.expander("Top-10 Candidate Sites — Full Statistics", expanded=False):
                top10_df = scr_df.nlargest(10, "mean_ws_hub")[[
                    scr_lat_key, scr_lon_key,
                    "mean_ws_10m", "mean_ws_hub", "p75_ws_hub", "cf_proxy_pct", "tier"
                ]].copy()
                top10_df.columns = [
                    "Latitude", "Longitude",
                    "Mean WS @10m (m/s)", f"Mean WS @{hub_height:.0f}m (m/s)",
                    f"P75 WS @{hub_height:.0f}m (m/s)", "Productive Hours (%)", "Tier"
                ]
                ...
                st.dataframe(
                    top10_df.style.format({
                        ...
                        "Longitude": "{:.4f}°",
                        "Mean WS @10m (m/s)": "{:.2f}",
                        f"Mean WS @{hub_height:.0f}m (m/s)": "{:.2f}",
                        f"P75 WS @{hub_height:.0f}m (m/s)": "{:.2f}",
                        "Productive Hours (%)": "{:.1f}",
                    }),
                    use_container_width=True,
                )
